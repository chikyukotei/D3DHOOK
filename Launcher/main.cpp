...
int _tmain(int argc, TCHAR* argv[])
{
	if(argc != 2)
	{
		std::cout<< "Usage: Launcher target-exe"<< std::endl
	...
	if(ERROR_SUCCESS != RegSetValueEx(regKey, 0, 0, REG_SZ, reinterpret_cast<BYTE*>(argv[1]),
		(_tcsclen(argv[1]) + 1) * sizeof(TCHAR)))
	{
