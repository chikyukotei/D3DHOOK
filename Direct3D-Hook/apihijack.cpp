...
bool RedirectIAT( SDLLHook* DLLHook, PIMAGE_IMPORT_DESCRIPTOR pImportDesc, PVOID pBaseLoadAddr )
{
    ...
    pIAT = MakePtr( PIMAGE_THUNK_DATA, pBaseLoadAddr, pImportDesc->FirstThunk ); // address of IAT(import address table)
    pINT = MakePtr( PIMAGE_THUNK_DATA, pBaseLoadAddr, pImportDesc->OriginalFirstThunk ); // address of INT(import name table)
        ...
        if ( !IMAGE_SNAP_BY_ORDINAL( pINT->u1.Ordinal ) )  // import by name
        {
            ...
            while ( FHook->Name )
            {
                if ( lstrcmpi( FHook->Name, (char*)pImportName->Name ) == 0 )
                {
                    ...
                    OutputDebugString( (char*)pImportName->Name ); // pImportName->Name = Direct3DCreate9
                    OutputDebugString( "\n" );
...
bool HookAPICalls( SDLLHook* Hook )
{
    ...
    HMODULE hModEXE = GetModuleHandle( 0 ); // DLL‚ðƒ[ƒh‚µ‚Ä‚¢‚éEXE
    ...
    while ( pImportDesc->FirstThunk )
    {
        ...
        if ( lstrcmpi( pszImportModuleName, Hook->Name ) == 0 )
        {
            ...
            OutputDebugString( Hook->Name ); // Hook->Name = D3D9.DLL
            OutputDebugString( "...\n" );
